...
def correct_correction(user_string, lm_string):
    client = LMStudioClient()
    system_prompt = """
    Входные данные: 
    - ВВОД 
    ...
    ВВОД - строка, которая содержит наименование товара и его параметры.
    Наименование товара - первое слое слово в строке. 

    ВЫВОД - строка, которая содержит наименование товара и его параметры.
    Наименование товара - первое слое слово в строке. 

    Проверь строки: 
    - если строка ВЫВОД не соответствует значениям из строки ВВОД, то напиши НЕВЕРНО 
    - если строка ВЫВОД соответствует значениям из строки ВВОД, то напиши ВЕРНО

    Пример: 
    лестничный марш железобетонный 1лм 30.12.15-4 бетон в22,5 объём 0,68 м3 расход арматура 18,31 кг
    марш лестничный железобетонный 1лм 30.12.15-4 бетон в22,5 объём 0,68 м3 расход арматура 18,31 кг
    ожидаемый вывод: ВЕРНО

    лестничный марш железобетонный 1лм 30.12.15-4 бетон в22,5 объём 0,68 м3 расход арматура 18,31 кг
    лист хризотилцементный волнистый профиль 40/150 7-волновой толщина 5,2 мм
    ожидаемый вывод: НЕВЕРНО

    Важно: 
    - В качестве вывода - ТОЛЬКО слова ВЕРНО и НЕВЕРНО
    - Не выводи ничего кроме одного слова
    - НЕ добавляй новых слов
    - Строго следуй инструкциям
    """

    answer = client.ask_question(f'{user_string}\n{lm_string}', system_prompt)
    return answer
